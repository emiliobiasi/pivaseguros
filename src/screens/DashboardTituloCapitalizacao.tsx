import { TituloCapitalizacaoTable } from "@/components/TituloCapitalizacaoTable/titulo-capitalizacao-table";
import { TituloCapitalizacao } from "@/types/TituloCapitalizacao";
import { useEffect, useState, useRef, useCallback } from "react";
import {
  fetchTituloCapitalizacaoList,
  subscribeToTituloCapitalizacaoUpdates,
  unsubscribeFromTituloCapitalizacaoUpdates,
...
export function DashboardTituloCapitalizacao() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { items, totalPages } = await fetchTituloCapitalizacaoList(
          page,
          limit,
          searchTerm,
  ...
  const handleTituloCapitalizacaoChange = useCallback(
    (e: RecordSubscription<TituloCapitalizacao>) => {
  ...
  useEffect(() => {
    subscribeToTituloCapitalizacaoUpdates(handleTituloCapitalizacaoChange);

    // Cancela a subscription quando o componente desmonta
    return () => {
      unsubscribeFromTituloCapitalizacaoUpdates();
    };
  }, [handleTituloCapitalizacaoChange]);
  ...
  return (
    <div>
      ...
      <div>
        ...
        {/* Table Component */}
        <TituloCapitalizacaoTable data={data} />
