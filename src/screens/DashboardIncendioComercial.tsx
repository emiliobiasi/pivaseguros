import { IncendioComercialTable } from "@/components/IncendioComercialTable/incendio-comercial-table";
import { SeguroIncendioComercial } from "@/types/SeguroIncendioComercial";
import { useEffect, useState, useRef, useCallback } from "react";
import {
  fetchSeguroIncendioComercialList,
  subscribeToSeguroIncendioComercialUpdates,
  unsubscribeFromSeguroIncendioComercialUpdates,
...
export function DashboardIncendioComercial() {
  const [data, setData] = useState<SeguroIncendioComercial[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { items, totalPages } = await fetchSeguroIncendioComercialList(
          page,
          limit,
          searchTerm,
        ...
        console.error("Erro ao buscar os seguros de incêndio comercial:", error);
      }
    };
    fetchData();
  ...
  const handleSeguroIncendioComercialChange = useCallback(
    (e: RecordSubscription<SeguroIncendioComercial>) => {
  ...
  useEffect(() => {
    subscribeToSeguroIncendioComercialUpdates(handleSeguroIncendioComercialChange);

    // Cancela a subscription quando o componente desmonta
    return () => {
      unsubscribeFromSeguroIncendioComercialUpdates();
    };
  }, [handleSeguroIncendioComercialChange]);
  ...
  return (
    <div>
      <TopBar
        title="Seguros de Incêndio Comercial"
      ...
      <div>
        ...
        {/* Table Component */}
        <IncendioComercialTable data={data} />
