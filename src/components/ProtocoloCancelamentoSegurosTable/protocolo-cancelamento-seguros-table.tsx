...
import { CancelamentoSegurosModal } from "../CancelamentoSegurosModal/protocolo-cancelamento-seguros-modal"
...
import {
  updateCancelamentoSegurosToPending,
  updateCancelamentoSegurosToFinalized,
...
export function ProtocoloCancelamentoSegurosTable({ data }: TableContentProps) {
  const [seguros, setSeguros] = useState<CancelamentoSeguros[]>([])
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [selectedSeguro, setSelectedSeguro] =
    useState<CancelamentoSeguros | null>(null)
  ...
  const ordenarPendentesPrimeiro = (seguros: CancelamentoSeguros[]) => {
  ...
  const handleAcaoChange = async (
    ...
    try {
      setLoadingAction(id)
      let updatedRecord: CancelamentoSeguros
      if (novaAcao === "PENDENTE") {
        updatedRecord = await updateCancelamentoSegurosToPending(id)
      } else {
        updatedRecord = await updateCancelamentoSegurosToFinalized(id)
      }
  ...
  const openUserModal = (seguro: CancelamentoSeguros) => {
