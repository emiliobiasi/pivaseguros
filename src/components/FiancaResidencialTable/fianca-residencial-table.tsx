...
export function FiancaResidencialTable({ data }: TableContentProps) {
  ...
  const ordenarPendentesPrimeiro = (seguros: SeguroFiancaResidencial[]) => {
  ...
  const handleAcaoChange = async (
    ...
    try {
      setLoadingAction(id);
      let updatedRecord: SeguroFiancaResidencial;
      if (novaAcao === "PENDENTE") {
        updatedRecord = await updateSeguroFiancaResidencialToPending(id);
      } else {
        updatedRecord = await updateSeguroFiancaResidencialToFinalized(id);
      }
