...
import { EfetivacaoSeguroFiancaModal } from "../EfetivacaoSeguroFiancaModal/efetivacao-seguro-fianca-modal";
...
import {
  updateEfetivacaoSeguroFiancaToPending,
  updateEfetivacaoSeguroFiancaToFinalized,
...
export function EfetivacaoSeguroFiancaTable({ data }: TableContentProps) {
  const [seguros, setSeguros] = useState<EfetivacaoSeguroFianca[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedSeguro, setSelectedSeguro] =
    useState<EfetivacaoSeguroFianca | null>(null);
  ...
  const ordenarPendentesPrimeiro = (seguros: EfetivacaoSeguroFianca[]) => {
  ...
  const handleAcaoChange = async (
    ...
    try {
      setLoadingAction(id);
      let updatedRecord: EfetivacaoSeguroFianca;
      if (novaAcao === "PENDENTE") {
        updatedRecord = await updateEfetivacaoSeguroFiancaToPending(id);
      } else {
        updatedRecord = await updateEfetivacaoSeguroFiancaToFinalized(id);
      }
  ...
  const openUserModal = (seguro: EfetivacaoSeguroFianca) => {
  ...
  return (
    ...
    >
      ...
      {selectedSeguro && (
        <EfetivacaoSeguroFiancaModal
          efetivacao={selectedSeguro}
          isOpen={isModalOpen}
          onClose={closeModal}
        />
      )}
