...
import { TituloCapitalizacaoModal } from "../TituloCapitalizacaoModal/titulo-capitalizacao-modal";
...
import {
  updateTituloCapitalizacaoToPending,
  updateTituloCapitalizacaoToFinalized,
} from "@/utils/api/TituloCapitalizacaoService";
...
export function TituloCapitalizacaoTable({ data }: TableContentProps) {
  const [seguros, setSeguros] = useState<TituloCapitalizacao[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedSeguro, setSelectedSeguro] = useState<TituloCapitalizacao | null>(
    null
  );
  ...
  const ordenarPendentesPrimeiro = (seguros: TituloCapitalizacao[]) => {
  ...
  const handleAcaoChange = async (
    ...
    try {
      setLoadingAction(id);
      let updatedRecord: TituloCapitalizacao;
      if (novaAcao === "PENDENTE") {
        updatedRecord = await updateTituloCapitalizacaoToPending(id);
      } else {
        updatedRecord = await updateTituloCapitalizacaoToFinalized(id);
      }
  ...
  const openUserModal = (seguro: TituloCapitalizacao) => {
